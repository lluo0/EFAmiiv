...
getbadvar_crossload <- function(fit, sigLevel=.05, num_fac, badvar=NULL){
  newbadvar_coef <- list()
...
select_scalingind_stepN <- function(data, sigLevel = .05,
                                    scalingCrit = "sargan+factorloading_R2", stepPrev){
...
step1_EFAmiiv <- function(data, sigLevel, scalingCrit = 'sargan+factorloading_R2', correlatedErrors  = NULL){
...
step2_EFAmiiv <- function(stepPrev, data, sigLevel, scalingCrit){
...
stepN_EFAmiiv <- function(stepPrev, data, sigLevel, scalingCrit){
...
#########final function#######
EFAmiiv <- function(data, sigLevel = .05, scalingCrit = 'sargan+factorloading_R2', correlatedErrors = NULL){
  step1 <- step1_EFAmiiv(data, sigLevel, scalingCrit, correlatedErrors)

  if(step1$nextstep == 'no'){
    finalobj <- step1
  }
  if(step1$nextstep == 'yes'){
    step2 <- step2_EFAmiiv(step1, data, sigLevel, scalingCrit )
    if(step2$nextstep == 'no'){
      finalobj <- step2
    }
    if(step2$nextstep =='yes'){
      stepN <- stepN_EFAmiiv(step2, data, sigLevel, scalingCrit)
      finalobj <- stepN
      while(stepN$nextstep =='yes'){
        stepN <- stepN_EFAmiiv(stepN, data, sigLevel, scalingCrit)
        finalobj <- stepN
      }
